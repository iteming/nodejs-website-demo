    ...
    return this.each(function () {
        ...
        */
        function drawLinks() {
            ...
            ///add by yangpan 2015-07-16 显示总条数
            panel.append("<span>共" + maxentries + "条<span>");
            panel.append("<span>第" + (np == 0 ? 0 : opts.current_page + 1) + "页/共" + np + "页<span>");


            // Helper function for generating a single link (or a span tag if it'S the current page)
            var appendItem = function (page_id, appendopts) {
                page_id = page_id < 0 ? 0 : (page_id < np ? page_id : (np <= 0 ? 0 : np - 1)); // Normalize page id to sane value
        ...
        //maxentries = (!maxentries && maxentries >= 0) ? maxentries : 1;
